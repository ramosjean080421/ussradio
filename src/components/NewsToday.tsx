...
export default function NewsToday() {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        const data: { items: NewsItem[]; pageCount?: number; error?: string } = await res.json();
  ...
  const proxied = (src?: string) => (src ? `/api/img?src=${encodeURIComponent(src)}` : undefined);
      ...
      <div className="mx-auto max-w-7xl px-4 py-16 md:px-6">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            ...
            <p className="mt-2 text-white/70">Titulares de Perú (El Comercio), actualizados automáticamente.</p>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-white/60">Página {page} de {pageCount}</span>
            <button
        ...
        <div className="mt-8 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {loading &&
            Array.from({ length: 6 }).map((_, i) => (
              <div key={`s${i}`} className="animate-pulse overflow-hidden rounded-2xl border border-white/10 bg-white/5">
          ...
          {!loading && err && (
            <div className="col-span-full rounded-xl border border-red-400/30 bg-red-400/10 p-4 text-red-200">{err}</div>
          )}

          {!loading && !err && items.map((n) => (
            <article key={n.id} className="group overflow-hidden rounded-2xl border border-white/10 bg-white/5">
              <div className="relative aspect-[16/10] w-full overflow-hidden bg-black/30">
                {n.image ? (
                  ...
                  <div className="grid h-full w-full place-items-center text-white/50">Sin imagen</div>
                )}
              ...
              <div className="p-4">
                ...
                {n.description && <p className="mt-2 text-sm text-white/70">{n.description}</p>}
              </div>
            </article>
          ))}
        </div>
