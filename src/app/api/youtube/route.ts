...
> {
  ...
  try {
    ...
    if (parts[0] === "c" && parts[1]) {
      // /c/BrandName -> suele redirigir a una página con el channelId en el HTML
...
export async function GET(req: Request) {
  ...
  // Aceptamos id | handle | channelId | env
  const raw =
    searchParams.get("id") ||
  ...
  try {
    ...
    if (!resolved) {
      return NextResponse.json(
        { mode: "rss", items: [], page: 1, pageCount: 1, error: "No se encontró el canal" },
        { status: 200 }
      );
    }
    ...
    return NextResponse.json(
      { mode: "rss", items, page: safePage, pageCount, error: null },
      { status: 200 }
    );
  } catch (e: any) {
    return NextResponse.json(
      { mode: "rss", items: [], page: 1, pageCount: 1, error: e?.message || "Error al cargar videos" },
      { status: 200 }
    );
  }
}
